...
class TestUpdateTodoItem(APITestCase):
	"""
	Ensure we can update a new todo item using PUT
	"""
	def setUp(self):
...
class TodoDeleteTodoItem(APITestCase):
	"""
	Ensure we can delete a new todo item using DELETE
	"""
	def setUp(self):
		response=createItem(self.client)
		self.assertEqual(TodoItem.objects.count(), 1)
...
class TodoDeleteAllItems(APITestCase):
	"""
	Ensure we can delete a new todo item using DELETE
	"""
	def setUp(self):
